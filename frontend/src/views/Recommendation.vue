  ...
  <v-container fluid pa-0 ma-0 style="height: 100%;">
    ...
    <v-card tile height="100%">
      <v-card class="mx-auto" max-width="900" flat>
        <v-card-title class="headline font-weight-medium">추천 여행지</v-card-title>
        <v-card-subtitle class="subtitle-1 font-weight-medium">어떤 여행지를 선택하시겠습니까?</v-card-subtitle>
        <v-alert color="#C51162" dark icon="mdi-heart" prominent>추천 여행지 중 하나를 선택하려면 해당 하트 버튼을 누르세요 (다시 누르면 선택이
          취소됩니다).</v-alert>
        <v-alert color="#C51162" dark icon="mdi-heart-broken" prominent>추천 여행지 중 마음에 드는 곳이 없다면 해당 버튼을 눌러주세요 (다시 누르면 선택이
          취소됩니다).</v-alert>
        <v-alert icon="mdi-information-outline" prominent text type="info" class="text-justify">추천 여행지에 대한 자세한 정보를 보려면
          해당 추천 여행지의 Wikipedia 또는 Google 버튼을 누르세요.</v-alert>
        <v-row dense>
          <v-col v-for="(r, index) in recommendations" :key="r.gid" cols="12" :md="column_size">
            <v-card class="rec-card" :disabled="!(selected_gid == 0 || selected_gid == r.gid)" outlined>
              ...
              <slick ref="slick" :options="slickOptions" @afterChange="increasePictureInteractionCount(index)">
                <img v-for="n in r.pic_num" :src="'/pics/' + r.gid + '/' + n + '.jpg'" :key="r.gid + '-' + n" />
              </slick>
              <v-btn fab dark :color='(selected_gid == r.gid) ? "#C51162" : "secondary"' absolute right top
                style="top: 5px; right: 5px;" @click="select(index, r.gid)">
                <v-icon dark>mdi-heart</v-icon>
              </v-btn>
              <v-btn fab dark small color="secondary" absolute right top
                style="top: 185px; right: 5px; opacity: 0.7; width: 40px;" @click="next(index)">
                <v-icon dark>mdi-chevron-right</v-icon>
              </v-btn>
              <v-btn fab dark small color="secondary" absolute right top
                style="top: 185px; left: 5px; opacity: 0.7; width: 40px;" @click="prev(index)">
                <v-icon dark>mdi-chevron-left</v-icon>
              </v-btn>
              <v-card-actions class="rec-card-actions">
                <v-btn color="primary" large icon :href="r.wiki_url" target="_blank"
                  @click="setInfoInteraction(true, index)">
                  <v-icon>mdi-wikipedia</v-icon>
                </v-btn>
                <v-btn color="primary" large icon :href="r.gtravel_url" target="_blank"
                  @click="setInfoInteraction(false, index)">
        ...
        <v-banner single-line>
          <v-btn :color='(selected_gid == -1) ? "#C51162" : "secondary"' large fab
            :disabled="!(selected_gid == 0 || selected_gid == -1)" @click="select(-1, -1)" :dark="(selected_gid == -1)">
            <v-icon>mdi-heart-broken</v-icon>
          </v-btn>
          <span style="margin-left: 10px;">마음에 드는 곳이 없으면 누르세요!</span>
        </v-banner>
        <v-card-actions>
          <v-spacer></v-spacer>
          <div class="my-2">
            <v-btn :disabled="(selected_gid == 0)" @click="finish()" large color="primary">완료</v-btn>
          </div>
        </v-card-actions>
      </v-card>
    </v-card>
